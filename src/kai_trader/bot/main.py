...
from kai_trader.bot.handlers import health, positions, start, status
...
async def _startup(_app: Application) -> None:  # type: ignore[type-arg]
    # Prime the DB pool so /health does not pay the connection cost on first use.
    await get_pool()
