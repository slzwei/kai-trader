...
def _parse_limit(args: str | None) -> int | str:
    """Return the parsed limit, or an error message string on bad input."""
...
async def _build(_update: Update, ctx: CommandContext) -> str:
    ...
    if not snapshots:
        return f"Account snapshots. {ts}\n\nNone recorded yet. Try /snapshot_now."

    lines = [f"Account snapshots, last {len(snapshots)}. {ts}", ""]
    for snap in snapshots:
        when = snap.captured_at.strftime("%Y-%m-%d %H:%M UTC")
        lines.append(
